...
class elnedyn22scfix:
    ...
    def __init__(self):
        ...
        # Most forcefields do not specify sidechain/Backbone/Backbone/sidechain dihedrals.
        # This variable does not need to specified if it is not True (we check for its excistence).
        self.UseSBBSDihedrals  = True
    ...
    def sbbsGetDihedral(self, ca, sc, fc=12.5):
        ''' Calculates the dihedral angle between the first side chain bead of two adjecent residues.'''
        import FUNC,IO 
        import math
        # Normal vectors to the planes formed by SC1-BB1-BB2 or SC2-BB2-BB1
        normal1 = FUNC.cross([i-j for i,j in zip(sc[0],ca[0])],[i-j for i,j in zip(ca[1],ca[0])])
        normal2 = FUNC.cross([i-j for i,j in zip(sc[1],ca[1])],[i-j for i,j in zip(ca[1],ca[0])])

        # Dihedral angle in degrees. In gromacs (0 degrees == cis).
        dih_angle = math.acos(FUNC.cos_angle(normal1,normal2))/IO.d2r

        # In Gromacs, dihedral function 1 is given by:           V = k* (1+cos(n*ang-ang0))
        # Which is different than that reported by Herzog et al: V= 0.5 * k * (1-cos(ang-ang0))
        # This means the force constant needed is half of that reported by Herzog et al. and
        # we need to shift the equilibrium angle by 180 degrees compared to what we measure.
        dih_angle = dih_angle-180. if dih_angle> 0 else dih_angle+180.
        return (dih_angle, fc)
    ...
    def messages(self):
        ...
        logging.warning('The scFix forcefield in martinize.py has not been tested.')
        logging.warning('For details on the forcefield, see (and cite): Herzog et al., J. Chem. Theory Comput., 2016, DOI:10.1021/acs.jctc.6b00122')
        pass
